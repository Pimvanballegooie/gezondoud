...
# -*- coding: utf-8 -*-
"""
Sync protocollen vanuit Google Docs — Gezond Oud Netwerk (gezondoud.net)
...
1. Leest protocollen-config.json:
   {
     "protocollen": [
       {
         "id": "valpreventie-thuis",
         "naam": "Valpreventie in en om huis",
         "zone": "valpreventie",          <- kwetsbaarheid | valpreventie | dementie | herstel-ziekenhuis | voorbereiding-operatie | revalidatie-operatie | sport-bewegen | levenslange-beperking
         "niveaus": {
...
# ══════════════════════════════════════════════════════════════════
# SITE-INSTELLINGEN — Gezond Oud Netwerk
# ══════════════════════════════════════════════════════════════════
SITE_URL     = "https://gezondoud.net"
SITE_NAAM    = "Gezond Oud Netwerk"
LOGO_BESTAND = "gezondoud_logo.png"
...
# Acht onderwerpen (categorie 2 "Ouderen" in Supabase) — geen thema-zone
ZONES = {
    "kwetsbaarheid":          "Kwetsbaarheid en mobiliteit",
    "valpreventie":           "Valpreventie en balans",
    "dementie":               "Dementie en bewegen",
    "herstel-ziekenhuis":     "Herstel na ziekenhuisopname",
    "voorbereiding-operatie": "Voorbereiding op operatie",
    "revalidatie-operatie":   "Revalidatie na operatie",
    "sport-bewegen":          "Sport, bewegen en overbelasting",
    "levenslange-beperking":  "Ouderen met levenslange beperking",
}
ZONE_ICONS = {
    "kwetsbaarheid": "🚶",
    "valpreventie": "⚖️",
    ...
    "revalidatie-operatie": "🔄",
    "sport-bewegen": "🚴",
    "levenslange-beperking": "♿",
}
ZONE_KLEUREN = {
    "kwetsbaarheid":          "#3D8B6E",
    "valpreventie":           "#5BAE7D",
    "dementie":               "#2E6B54",
    "herstel-ziekenhuis":     "#2E7A8C",
    "voorbereiding-operatie": "#4FA396",
    "revalidatie-operatie":   "#3E7A9E",
    "sport-bewegen":          "#E8A13A",
    "levenslange-beperking":  "#6FA285",
}
...
def haal_doc_tekst(doc_id: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "GezondOudNetwerk-Sync/1.0"})
...
def header_html(root_pad: str = "") -> str:
  ...
  <div class="header-inner">
    <a href="{root_pad}index.html" class="logo">
      ...
      <div class="logo-text"><span>Gezond Oud</span> Netwerk</div>
    </a>
...
def bouw_protocol_pagina(protocol: dict, niveau: str, inhoud_html: str) -> str:
    ...
    if is_patient:
      ...
      ℹ️ Deze uitleg hoort bij een behandelprotocol van het Gezond Oud Netwerk en is geen vervanging
...
window.addEventListener('beforeunload', () => speechSynthesis.cancel());
      ...
      ⚕️ Klinische versie voor aangesloten therapeuten van het Gezond Oud Netwerk. Toepassing vraagt
...
<head>
  ...
  <style>
    ...
    .voorlees-balk {{ display: flex; align-items: center; gap: 12px; background: var(--teal-light); border: 1px solid rgba(61,139,110,0.3); border-radius: 12px; padding: 12px 16px; margin-bottom: 22px; }}
...
def bouw_overzicht(protocollen: list, previews: dict) -> str:
    zone_filters = '<button class="zone-filter actief" data-zone="alle" onclick="filterZone(this)">Alle onderwerpen</button>'
    ...
    if protocollen:
      ...
      <p style="color:var(--text-muted);font-size:0.9rem;max-width:440px;margin:0 auto">
        Onze expertteams werken aan de eerste behandelprotocollen voor kwetsbaarheid, valpreventie, dementie, herstel na opname en operatie.
...
<head>
  ...
  <meta name="description" content="Behandelprotocollen van het {SITE_NAAM}: kwetsbaarheid, valpreventie, dementie, herstel na ziekenhuisopname, operatie-voorbereiding en -revalidatie, sport en levenslange beperkingen. Elk protocol in een leesbare versie voor patiënten en een klinische versie voor therapeuten." />
  <style>
    {basis_css()}
    .hero {{ background: linear-gradient(160deg, #5BAE7D 0%, #3D8B6E 50%, #2E7A8C 100%); position: relative; color: white; text-align: center; padding: 56px 24px 48px; }}
    ...
    .zoek-input:focus {{ border-color: var(--teal); box-shadow: 0 0 0 3px rgba(61,139,110,0.12); }}
...
<div class="pagina">
  <div class="zoek-rij">
    <input type="text" class="zoek-input" id="zoek-input" placeholder="🔍 Zoek bijv. valpreventie, dementie, operatie..." oninput="filterProtocollen()" />
  </div>
